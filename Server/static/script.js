...
async function loadSong(song) {
    ...
    try {
        ...
        if (audioData.source === 'local') {
            ...
            const headersParam = audioData.headers ? `&headers=${encodeURIComponent(JSON.stringify(audioData.headers))}` : '';
            audioPlayer.src = `/stream_proxy?url=${encodeURIComponent(audioData.url)}${headersParam}`;
        }
